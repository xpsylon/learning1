...
while check :
    ...
    semana = ('lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo') #tupla dias de semana
    ...
    if dia_numero > 31:
        ...
        print('Mes erroneo')
    elif dia_semana not in semana:
        print('Dia de la semana erroneo')    
    else:
        check = False
        print(fecha)
        #condiciones 2
        if dia_semana in semana[0:3]:
            examen = input('Hubo examenes? Y para si, N para no: ')
            if examen.upper() == 'Y':
            ...
            if dia_numero == 1 and (mes == 1 or mes == 7):
                ...
                print('Ingreso total nuevo ciclo', ingreso_total, 'EUR')
            else:
                print('Inscripciones solo 01/01 y 01/07')
        else:
            check = True
            print('El fin de semana permanecemos cerrados')
